...
import { AddExerciseBodyPartAPI } from "../../services/ApiServices";

const AddBodyPartModal = ({ isDark, onClose, onAdded }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!name.trim()) {
      setError("Please enter a body part / muscle name");
      return;
    }
    setLoading(true);
    setError(null);
    try {
      const data = await AddExerciseBodyPartAPI({ name: name.trim() });
      if (onAdded) {
        onAdded(data);
      }
      if (onClose) {
      ...
      setError(
        typeof err === "string"
          ? err
          : err?.error || err?.message || "Failed to add body part / muscle"
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Create a new body part / muscle group. Later you can link exercises to it.
        </p>
...
export default AddBodyPartModal;
