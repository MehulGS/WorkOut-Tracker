...
import { AddExerciseAPI } from "../../services/ApiServices";

const AddExerciseModal = ({ isDark, bodyPartId, onClose, onAdded }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!bodyPartId) {
      setError("Missing body part id");
      return;
    }
    setLoading(true);
    setError(null);
    try {
      const data = await AddExerciseAPI({ bodyPartId, name: name.trim() });
      if (onAdded) {
        onAdded(data);
      }
      if (onClose) {
      ...
      setError(
        typeof err === "string"
          ? err
          : err?.error || err?.message || "Failed to add exercise"
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Create a new exercise for this body part / muscle group.
        </p>
